...
membrane.create = function(initTarget, moduleName) {
  ...
  var wrap = function(target, objectName) {
    ...
    var handler = {
      ...
      apply : function (target, thisArg, argumentsList) {
        ...
        if (target == Function.prototype.toString) {
          ...
          if (mapWrapped2Unwrapped.has(thisArg)) {
            fCall = Reflect.apply(target, mapWrapped2Unwrapped.get(thisArg), argumentsList);
          } else {
            fCall = Reflect.apply(target, thisArg, argumentsList);
          }
        }
    ...
    mapWrapped2Unwrapped.set(wrappedTarget, objectToBeWrapped);
    mapUnwrapped2Wrapped.set(objectToBeWrapped, wrappedTarget);
